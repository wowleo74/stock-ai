    ...
    try:
        ...
        for i, (name, ticker) in enumerate(tickers.items()):
            ...
            if len(s) >= 2:
                ...
                diff = curr - prev
                rate = (diff / prev) * 100
                color = "#e03131" if diff > 0 else ("#1971c2" if diff < 0 else "#495057")
                ...
                with cols[i]:
                    st.markdown(f"""
                        <div class="index-card">
                            ...
                            <div class="index-change" style="color:{color};">{"▲" if diff > 0 else "▼"} {abs(diff):,.2f} ({rate:+.2f}%)</div>
                        </div>
                    """, unsafe_allow_html=True)
    except Exception:
...
def get_detailed_info(symbol):
    try:
        ...
        if not info or 'marketCap' not in info:
            info['marketCap'] = getattr(ticker.fast_info, 'market_cap', 0)
            info['regularMarketPreviousClose'] = getattr(ticker.fast_info, 'previous_close', 0)
        return info
    except:
        return {}
...
if st.session_state.page_selection == "📊 단일 종목 분석":
    ...
    if df is not None:
        ...
        mode_label_map = {"UPTREND": "🚀 상승 추세장", "SIDEWAYS": "⚖️ 박스 횡보장", "VOLATILE_UP": "🔥 변동성 장세", "DOWNTREND": "📉 하락장"}
        mode_colors = {
        ...
        st.markdown(f"""
            <div style='background-color:#ffffff; padding:25px; border-radius:15px; border:2px solid #e9ecef; margin-bottom:20px;'>
                <div>
                    ...
                    {f"<span class='weight-badge'>⚖️ 권장 투자 비중: {rec_weight*100:.0f}%</span>" if rec_weight > 0 else ""}
...
# ====== 화면 2: 스캐너 (관심종목 기능 탑재 / 10개 출력) ======
    ...
    if st.session_state.search_results:
        ...
        for item in st.session_state.search_results:
            ...
            with c_info: 
                st.markdown(f"<div class='rank-card'><b>{item['Name']}</b> ({item['Code']}) <br> {item['Status']} | 점수: {item['Score']} | 비중: {item['Weight']}%</div>", unsafe_allow_html=True)
            with c_btn:
                st.write("")
    ...
    if not pf: 
        ...
        for code, data in pf.items():
            ...
            if res[0] is not None: 
                ...
                curr_price, c_atr, sl_mult, tp_mult = data['BuyPrice'], 0, 1.5, 2.5
            ...
            if curr_price >= my_tp: 
                ai_action, card_class, act_color = "🚀 수익 실현 권장", "pf-profit", "#5c940d"
            elif curr_price <= my_sl: 
                ai_action, card_class, act_color = "⚠️ 칼손절 필요", "pf-loss", "#e03131"
            else: 
                ai_action, card_class, act_color = "✅ 홀딩 (관망)", "portfolio-card", "#2b8a3e"
...
# ====== 화면 5, 6: 관심종목 & 백과사전 ======
    ...
    st.info("자금 관리, 국면 스위칭 등 핵심 전략이 탑재되어 있습니다.")